$(function () {
    ...
    $X_Tableb.XGrid({
        ...
        colNames: ['', '单据编号', '单据日期', '单据类型', '单据状态', '是否监管', '采购订单编号', '商品大类', '商品编号', '商品规格',
            '生产厂家', '包装单位', '收货数', '是否特殊药品', '剂型', '产地', '批号', '生产日期', '有效日期', '容器编码', '单价',
            '总金额', '中包装规格', '见包装规格', '收获零散数', '收获件数', '收货结论', '处理原因', '在途数量', '预计到货日期', '单据状态',
            '收货结论', '收货备注', '验收结论', '验收备注', '复查结论', '复查备注', '修改备注', '拒收货位', '拒收备注', '单据创建时间',
            '单据完成时间', '单据操作'
        ],
        colModel: [{
            name: 'id'
    ...
    $('#supplierName').dblclick(function () {
        utils.dialog({
            title: '供应商列表',
            url: 'Job_form/Supplier_selection.html',
    ...
    $('#commodityName').dblclick(function () {
        utils.dialog({
            title: '供应商列表',
            url: 'Job_form/Commodity_selection.html',
            width: $(window).width() * 0.8,
            height: 600,
            data: $('#product').val(), // 给modal 要传递的 的数据
            onclose: function () {
