!(function () {
    ...
    var gridData = [];
    var colNames = ['业主编码', '业主名称', '商品编码', '商品名称', '商品规格', '生产厂家','产地', '单位', '包装数量', '批号',
                    '生产日期','有效期至','验收评定','采取措施','库别名称','退回数量','抽检数量', '实际退回数量', '实际件数', '实际零散数',
                    '容器编号', '退货原因'];
    var colModels = [
        {
            name: 'name1'
    ...
    //验收日期
    $('#checkTime').val(moment().format('YYYY-MM-DD'));
    ...
    function splitRowData(Xgrid, obj) {
        ...
        $('#split_table').XGrid({
            data: [],
            colNames: ['是否监管', '商品编号', '商品名称', '商品规格', '生产厂家', '包装单位', '单价', '容器编号', '验收评定', '采取措施', '库别名称','件包装规格','行数量','总数量'],
            colModel: [
                {
                    name: 'name1'
                },
